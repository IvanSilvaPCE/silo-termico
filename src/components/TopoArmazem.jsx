...
const TopoArmazem = ({ onArcoSelecionado, arcoAtual, onFecharTopo }) => {
    ...
    useEffect(() => {
        const processarDados = async () => {
            try {
                ...
                const layoutFallback = gerarLayoutAutomatico({
    ...
    function criarSVGTopo() {
        ...
        svgEl.setAttribute(
            "viewBox",
            `0 0 ${layoutTopo.celulas.tamanho_svg[0]} ${layoutTopo.celulas.tamanho_svg[1]}`,
        );
        svgEl.setAttribute(
            "style",
            "background: #f8f9fa; border-radius: 8px; shape-rendering:geometricPrecision; text-rendering:geometricPrecision;",
        );
    ...
    function desenharArco(idArco, dadosArco) {
        ...
        // Grupo do arco
        const grupoArco = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "g",
        );
        ...
        const retanguloSelecao = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect",
        );
        ...
        // Botão superior
        const botaoSup = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect",
        );
        ...
        const textoSup = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "text",
        );
        ...
        // Botão inferior
        const botaoInf = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "rect",
        );
        ...
        const textoInf = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "text",
        );
    ...
    function desenharCabo(idCabo, posX, posY) {
        ...
        const circuloPQ = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "circle",
        );
        ...
        const animacao = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "animate",
        );
        ...
        const circuloFalha = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "circle",
        );
    ...
    function desenharAerador(idAerador, posX, posY, textoAcima) {
        ...
        const grupoBladePrado = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "g",
        );
        grupoBladePrado.setAttribute("id", `blade_aerador_${idAerador}_parado`);

        const grupBladeGirando = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "g",
        );
        grupBladeGirando.setAttribute(
            "id",
            `blade_aerador_${idAerador}_girando`,
        );
        ...
        // Blade parada
        const bladeParada = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path",
        );
        ...
        // Blade girando
        const bladeGirando = document.createElementNS(
            "http://www.w3.org/2000/svg",
            "path",
        );
    ...
    function atualizarSelecoes() {
        ...
        const arcoSelecionadoRect = document.getElementById(
            `rec_arco_${arcoSelecionado}`,
        );
        ...
        Object.keys(layoutTopo).forEach((key) => {
            if (key !== "celulas" && key !== "aeradores") {
                const arcoNum = parseInt(key);
                const botaoSup = document.getElementById(
                    `arco${arcoNum}_botsup`,
                );
                const botaoInf = document.getElementById(
                    `arco${arcoNum}_botinf`,
                );

                if (botaoSup && botaoInf) {
                    const cor =
                        arcoNum === arcoSelecionado ? "#33CC33" : "#999999";
    ...
    function atualizarCabos() {
        Object.entries(dadosTopo).forEach(([idCabo, dados]) => {
            ...
            if (circulo && texto) {
                ...
                if (pontoQuenteEl) {
                    pontoQuenteEl.style.visibility =
                        pontoQuente && !falha ? "visible" : "hidden";
                }
            }
        });
    }
    ...
    function setarEstadoAerador(aeradorId, estado) {
        ...
        const bladePrado = document.getElementById(
            `blade_aerador_${aeradorId}_parado`,
        );
        const bladeGirando = document.getElementById(
            `blade_aerador_${aeradorId}_girando`,
        );
        ...
        svgEl.addEventListener("click", (evento) => {
            ...
            if (grupo && grupo.id) {
                ...
                if (tipo === "arco") {
                    ...
                    Object.entries(layoutTopo).forEach(
                        ([arcoKey, arcoData]) => {
                            if (
                                arcoKey !== "celulas" &&
                                arcoKey !== "aeradores"
            ...
            if (elemento.id && elemento.id.startsWith("rec_celula")) {
                const numeroCelula = parseInt(
                    elemento.id.replace("rec_celula", ""),
                );
                if (numeroCelula > 0) {
