<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Tipos de alarmes cadastrados</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-tipo-alarme" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-tipo-alarme" title="Excluir Tipo de Alarme?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_tipo_alarme }}) {{ itemSelecionado.nm_tipo_alarme }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_tipo_alarme" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarTipoAlarme" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovoTipoAlarme.nm_tipo_alarme" :state="validate('nm_tipo_alarme')"></b-form-input>
               <b-form-invalid-feedback id="nm_tipo_alarme">{{ erros['nm_tipo_alarme'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
            <b-form-group label="Altera Status">
               <v-select v-model="objNovoTipoAlarme.fl_st_operacao" :options="optionsFlOperacao" :state="validate('fl_st_operacao')" :reduce="(operacao) => operacao.value" :class="{ 'border-danger': validate('fl_st_operacao') == false }">
                  <template #no-options="">Nenhuma correspondência</template>
               </v-select>
               <small class="text-danger">{{ erros['fl_st_operacao'] | parseErros }}</small>
            </b-form-group>
         </form>
...
import { tipoAlarmeService } from '../services'

export default {
   name: 'CadastroTipoAlarme',
   data() {
      return {
         isExcluindo: false,
         filter: null,
         ...
         fields: [
            { key: 'id_tipo_alarme', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'nm_tipo_alarme', label: 'Descrição' },
            { key: 'fl_st_operacao', label: 'Altera Status', formatter: 'formatarStatus' }
         ],
         optionsFlOperacao: [
            { value: 'S', label: 'Sim' },
            { value: 'N', label: 'Não' }
   ...
   methods: {
      ...
      formNovo() {
         this.erros = []
         this.objNovoTipoAlarme = {}
         this.tituloModal = 'Cadastrar Tipo de Alarme'
         this.$bvModal.show('cadastro')
      },
      async salvarTipoAlarme(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await tipoAlarmeService.salvarTipoAlarme(this.objNovoTipoAlarme)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovoTipoAlarme = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Tipo de Alarme'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione um tipo de alarme na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_tipo_alarme
         const result = await client.delete(`/tipo_alarme/${id}`)
         if (result.data == true) {
            this.showMessage({
