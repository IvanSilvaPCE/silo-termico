<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Riscos de alarmes cadastrados</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-risco" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-risco" title="Excluir Risco?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_risco }}) {{ itemSelecionado.nm_risco }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_risco" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarRisco" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovoRisco.nm_risco" :state="validate('nm_risco')"></b-form-input>
               <b-form-invalid-feedback id="nm_risco">{{ erros['nm_risco'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_risco', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'nm_risco', label: 'Descrição' }
         ]
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoRisco = {}
         this.tituloModal = 'Cadastrar Risco'
         this.$bvModal.show('cadastro')
      },
      async salvarRisco(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await riscoService.salvarRisco(this.objNovoRisco)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovoRisco = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Risco'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione um risco na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_risco
         const result = await client.delete(`/risco/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Risco',
