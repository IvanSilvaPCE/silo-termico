<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Lotes de destino cadastrados</h2>
               </div>
            </div>
               ...
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-lote" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-lote" title="Excluir Lote de Destino?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_lote_destino }}) {{ itemSelecionado.descricao }}
                              <hr />
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarLote" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Código">
               <b-form-input v-model="objNovoLote.codigo" :state="validate('codigo')"></b-form-input>
               <b-form-invalid-feedback id="codigo">{{ erros['codigo'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovoLote.descricao" :state="validate('descricao')"></b-form-input>
               <b-form-invalid-feedback id="descricao">{{ erros['descricao'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
import { loteDestinoService } from '../services'

export default {
   name: 'CadastroLoteDestino',
   data() {
      return {
         idSecador: localStorage.getItem('sec'),
         isExcluindo: false,
         filter: null,
         tituloModal: '',
         selecionado: [],
         ...
         fields: [
            { key: 'codigo', label: 'Código', tdClass: 'text-center', class: 'text-center' },
   ...
   computed: {
      ...
      itemsFiltrados() {
         ...
         return this.items.filter((item) => {
            const keys = Object.keys(item).filter((key) => key !== 'id_lote_destino')
            if (String(item.codigo).toLowerCase().includes(filterLower)) {
               return true
            }
            if (String(item.descricao).toLowerCase().includes(filterLower)) {
               return true
            }
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoLote = {}
         this.tituloModal = 'Cadastrar Lote de Destino'
         this.$bvModal.show('cadastro')
      },
      async salvarLote(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await loteDestinoService.salvarLote(this.objNovoLote)
         switch (status) {
            case 200:
      ...
      async buscar() {
         this.isBusy = true
         const dados = await client.get(`/lote_destino/buscarlote?equipamento=${this.idSecador}`).catch((err) => {
      ...
      editar() {
         this.objNovoLote = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Lote de Destino'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione um lote de Destino',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_lote_destino
         const result = await client.delete(`/lote_destino/${id}`)
         if (result.data == true) {
            this.showMessage({
