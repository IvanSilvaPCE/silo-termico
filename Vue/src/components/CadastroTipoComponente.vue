<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Tipos de componentes cadastradas</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-tipo" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-tipo" title="Excluir Tipo de Componente?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_tipo_componente }}) {{ itemSelecionado.ds_tipo_componente }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_tipo_componente" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarTipoComponente" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovoTipoComponente.ds_tipo_componente" :state="validate('ds_tipo_componente')"></b-form-input>
               <b-form-invalid-feedback id="ds_tipo_componente">{{ erros['ds_tipo_componente'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
import { tipoComponenteService } from '../services'

export default {
   name: 'CadastroTipoComponente',
   data() {
      return {
         isExcluindo: false,
         filter: null,
         ...
         fields: [
            { key: 'id_tipo_componente', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'ds_tipo_componente', label: 'Descrição' }
         ]
      }
   },
   ...
   computed: {
      ...
      itemsFiltrados() {
         ...
         return this.items.filter((i) => {
            ...
            keys.splice(keys.indexOf('id_tipo_componente'), 1)
            return keys.find((k) => String(i[k]).includes(this.filter))
         })
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoTipoComponente = {}
         this.tituloModal = 'Cadastrar Tipo Componente'
         this.$bvModal.show('cadastro')
      },
      async salvarTipoComponente(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await tipoComponenteService.salvarTipoComponente(this.objNovoTipoComponente)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovoTipoComponente = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Tipo Componente'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione um tipo de componente na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_tipo_componente
         const result = await client.delete(`/tipo_componente/${id}`)
         if (result.data == true) {
            this.showMessage({
