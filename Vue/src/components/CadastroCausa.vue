<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Causas de alarmes cadastradas</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-causa" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-causa" title="Excluir Causa?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_causa }}) {{ itemSelecionado.ds_causa }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_causa" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarCausa" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovaCausa.ds_causa" :state="validate('ds_causa')"></b-form-input>
               <b-form-invalid-feedback id="ds_causa">{{ erros['ds_causa'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
import { causaService } from '../services'
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_causa', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'ds_causa', label: 'Descrição' }
         ]
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovaCausa = {}
         this.tituloModal = 'Cadastrar Causa'
         this.$bvModal.show('cadastro')
      },
      async salvarCausa(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await causaService.salvarCausa(this.objNovaCausa)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovaCausa = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Causa'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione uma causa na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_causa
         const result = await client.delete(`/causa/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Causa',
