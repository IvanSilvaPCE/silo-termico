<template>
   <div class="align-items-center">
      <b-row v-show="this.dadosArco?.[1]">
         <b-col class="text-center">
            <label for="sb-large">Selecionar arco</label>
            <b-form-spinbutton v-model="arcoSelecionado" min="1" :max="maxArco" size="md" class="mb-4"></b-form-spinbutton>
         </b-col>
      </b-row>
      <b-row>
         <b-col>
            <div id="desenho-arco"></div>
         </b-col>
      </b-row>
   </div>
</template>
...
export default {
   ...
   computed: {
      maxArco() {
         const chaves = Object.keys(this.dadosArco)
   ...
   methods: {
      ...
      enviaArco() {
         this.$emit('receberArco', this.arcoSelecionado)
      },
      recebeDadosArco(dadosArco) {
         this.dadosArco = dadosArco
      },
      recebeConfigArco(configArco) {
         this.configArco = configArco
      },
      desenhaArco(_layout_arco) {
         let _desenho
         ...
         const container = document.getElementById('desenho-arco')
         container.appendChild(_desenho)
         //seleciona o layout do arco
         this.desenhaFundo(_layout_arco)
         //desenha sensores
         this.desenhaSensores(_layout_arco)
      },
      atualizar(_dados_arco) {
         let _id_cabos, _num_cabos, _num_sensores, _cabo, _sensor, _des_tex_nome_cabo, _des_rec_temperatura, _des_tex_temperatura, _des_rec_ponto_quente, _des_rec_erro, _falha_sensor
         ...
         for (_cabo = 1; _cabo < _num_cabos + 1; _cabo++) {
            ...
            for (_sensor = 1; _sensor <= _num_sensores; _sensor++) {
               ...
               if (this.layout_arco['desenho_sensores'].escala_cores_sensores == 1) {
                  //temperatura >= 35 e < 50
               ...
               if (_falha_sensor == true) {
                  ...
                  _des_tex_temperatura.setAttribute('font-size', String(this.layout_arco['desenho_sensores'].escala_sensores * 0.4 - 1.5))
      ...
      desenhaFundo(_dados_arco) {
         ...
         _path_base = String(_p1[0]) + ',' + String(_p1[1]) + ' ' + String(_p2[0]) + ',' + String(_p2[1]) + ' ' + String(_p3[0]) + ',' + String(_p3[1]) + ' ' + String(_p4[0]) + ',' + String(_p4[1]) + ' ' + String(_p5[0]) + ',' + String(_p5[1]) + ' ' + String(_p6[0]) + ',' + String(_p6[1]) + ' ' + String(_p7[0]) + ',' + String(_p7[1]) + ' ' + String(_p8[0]) + ',' + String(_p8[1]) + ' '
         ...
         if (_tipo_telhado == 1) {
            ...
            //forma a string com o path
            _path_telhado = String(_p1[0]) + ',' + String(_p1[1]) + ' ' + String(_p2[0]) + ',' + String(_p2[1]) + ' ' + String(_p3[0]) + ',' + String(_p3[1]) + ' ' + String(_p4[0]) + ',' + String(_p4[1]) + ' ' + String(_p5[0]) + ',' + String(_p5[1]) + ' ' + String(_p6[0]) + ',' + String(_p6[1]) + ' ' + String(_p7[0]) + ',' + String(_p7[1]) + ' '
         ...
         //telhado arco
            ...
            //forma a string com o path
            _path_telhado = 'M' + String(_p1[0]) + ',' + String(_p1[1]) + ' ' + 'L' + String(_p2[0]) + ',' + String(_p2[1]) + ' ' + String(_p3[0]) + ',' + String(_p3[1]) + ' ' + 'C' + String(_p4[0]) + ',' + String(_p4[1]) + ' ' + String(_p5[0]) + ',' + String(_p5[1]) + ' ' + String(_p6[0]) + ',' + String(_p6[1]) + ' ' + String(_p7[0]) + ',' + String(_p7[1]) + ' ' + String(_p8[0]) + ',' + String(_p8[1]) + ' ' + String(_p9[0]) + ',' + String(_p9[1]) + ' ' + 'L' + String(_p10[0]) + ',' + String(_p10[1]) + ' ' + String(_p11[0]) + ',' + String(_p11[1]) + ' z'
            //cria desenho do telhado
   ...
   watch: {
      dadosArco() {
         this.layout_arco = this.configArco.find((item) => item.numero.includes(this.arcoSelecionado))?.layout_arco

         this.desenhaArco(this.layout_arco)
         this.atualizar(this.dadosArco[this.arcoSelecionado])
      },
      arcoSelecionado(newValue, oldValue) {
         if (newValue != oldValue) {
            this.enviaArco()
         }
         this.layout_arco = this.configArco.find((item) => item.numero.includes(this.arcoSelecionado))?.layout_arco
         this.desenhaArco(this.layout_arco)
         this.atualizar(this.dadosArco[newValue])
      }
   }
}
</script>
