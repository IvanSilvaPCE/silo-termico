<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Grupos de alarmes cadastrados</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-grupo" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-grupo" title="Excluir Grupo?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_grupo_alarme }}) {{ itemSelecionado.nm_grupo_alarme }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_grupo_alarme" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarGrupo" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovoGrupo.nm_grupo_alarme" :state="validate('nm_grupo_alarme')"></b-form-input>
               <b-form-invalid-feedback id="nm_grupo_alarme">{{ erros['nm_grupo_alarme'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_grupo_alarme', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'nm_grupo_alarme', label: 'Descrição' }
         ]
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoGrupo = {}
         this.tituloModal = 'Cadastrar Grupo'
         this.$bvModal.show('cadastro')
      },
      async salvarGrupo(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await grupoService.salvarGrupo(this.objNovoGrupo)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovoGrupo = { ...this.itemSelecionado }
         this.erros = []
      ...
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione um grupo na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_grupo_alarme
         const result = await client.delete(`/grupo_alarme/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Grupo',
