<template>
   <div class="" ref="chartContainer">
      <b-overlay :show="buscandoTemperaturas || buscandoPressoes || buscandoUmidades || buscandoParametros || buscandoAcionamentos" rounded="sm" class="overlay">
         <b-row v-if="filtroGrafico != 'parametro'" class="mb-3 align-items-end">
            <b-col lg="5">
            ...
            <b-col lg="2">
               <b-button variant="outline-primary" class="mt-2" @click="acionaBusca(filtroGrafico)" :disabled="buscandoTemperaturas || buscandoPressoes || buscandoUmidades || buscandoParametros || buscandoAcionamentos">
                  <span v-if="!buscandoTemperaturas && !buscandoPressoes && !buscandoUmidades && !buscandoParametros && !buscandoAcionamentos">Filtrar</span>
                  <div v-else>
         ...
         <b-overlay :show="this.verificarObjetoVazio() && filtroGrafico != 'parametro'" opacity="0">
            <template #overlay>
               <div class="text-center text-nowrap">
                  <p>Sem dados no período</p>
               </div>
            </template>
            <b-row v-if="filtroGrafico == 'temperatura' || filtroGrafico == 'todos' || filtroGrafico == 'pressao' || filtroGrafico == 'umidade'">
               <b-col>
                  <label>Últimos valores (clique para detalhes)</label>
               </b-col>
            </b-row>
            <b-row v-if="filtroGrafico == 'temperatura' || filtroGrafico == 'todos'" class="justify-content-center d-flex overflow-auto">
               <b-col cols="12">
                  <b-list-group horizontal>
                     <b-list-group-item v-for="chave in chavesTemperatura" :key="chave" @click="graficoVariavel((variavel = chave), modo)" class="d-flex justify-content-between align-items-center cursor">
                        <b-badge variant="primary" class="valor mr-1">{{ chave.toUpperCase() }}</b-badge>
                        <small class="valor">{{ temperaturas[chave.replace(/s/g, 't')][temperaturas[chave.replace(/s/g, 't')].length - 1]?.[1] }}</small
                        ><small class="ml-1">°C</small>
                     </b-list-group-item>
                  </b-list-group>
               </b-col>
            </b-row>
            <b-row v-if="filtroGrafico == 'pressao'" class="justify-content-center d-flex overflow-auto">
               <b-col cols="12">
                  <b-list-group horizontal>
            ...
            <b-row v-if="filtroGrafico == 'umidade'">
               <b-col sm="12" lg="4" class="text-center">
                  <small class="valor">Umidades</small>
                  <b-list-group>
                     <b-list-group-item @click="graficoVariavel((variavel = 'um'), modo)" class="d-flex justify-content-between align-items-center p-2 cursor">
                        <b-badge variant="primary" class="valor mr-1">U1</b-badge>
                        <small class="valor">{{ umidades?.ue?.filter((u) => u[1] !== '-').slice(-1)[0]?.[1] }} %</small>
                     </b-list-group-item>
                     <b-list-group-item @click="graficoVariavel((variavel = 'um'), modo)" class="d-flex justify-content-between align-items-center p-2 cursor">
                        <b-badge variant="primary" class="valor mr-1">U2</b-badge>
                        <small class="valor">{{ umidades?.us?.filter((u) => u[1] !== '-').slice(-1)[0]?.[1] }} %</small>
                     </b-list-group-item>
                  </b-list-group>
               </b-col>
               <b-col sm="12" lg="4" class="text-center">
                  <small class="valor">Produção</small>
                  <b-list-group>
                     <b-list-group-item @click="graficoVariavel((variavel = 'prod'), modo)" class="d-flex justify-content-between align-items-center p-2 cursor" v-b-tooltip.hover title="Descarga">
                        <b-badge variant="primary" class="valor">Desc.</b-badge>
                        <small class="valor">{{ umidades?.vdp?.filter((u) => u[1] !== '-').slice(-1)[0]?.[1] }} %</small>
                     </b-list-group-item>
                     <b-list-group-item @click="graficoVariavel((variavel = 'prod'), modo)" class="d-flex justify-content-between align-items-center p-2 cursor" v-b-tooltip.hover title="Produção">
                        <b-badge variant="primary" class="valor">Prod.</b-badge>
                        <small class="valor">{{ umidades?.di?.filter((u) => u[1] !== '-').slice(-1)[0]?.[1] }} ton/h</small>
                     </b-list-group-item>
                  </b-list-group>
               </b-col>
            </b-row>
            <b-row v-if="filtroGrafico != 'parametro'">
               <b-col class="mt-2">
                  <small>Ferramentas do gráfico</small>
               </b-col>
            </b-row>
            <b-row>
               <b-col v-if="filtroGrafico != 'parametro'" class="pb-0">
            ...
            <div ref="chart" v-show="this.filtroGrafico != 'parametro'" style="width: 100%; height: 71vh"></div>
            ...
            <b-row v-if="variavel || filtroGrafico == 'acionamento'">
               <b-col v-if="modo == 'fluxo' && filtroGrafico != 'acionamento'" class="d-flex align-items-center mt-2">
               ...
               <b-col v-if="modo == 'operacao' || filtroGrafico == 'acionamento'" class="d-flex align-items-center mt-2">
               ...
               <b-col v-if="modo == 'operacao' || filtroGrafico == 'acionamento'" class="d-flex align-items-center mt-2">
                  <b-badge class="pr-4 pb-3 mr-1" style="background-color: rgba(247, 96, 28, 0.4)">&nbsp;</b-badge>
            ...
            <b-row v-if="filtroGrafico == 'parametro' && !this.isDataExpirada()" class="text-center justify-content-center d-flex overflow-auto">
            ...
            <b-row v-if="filtroGrafico == 'parametro' && !this.isDataExpirada()" class="m-2">
                     ...
                     <div class="card-body pt-0 pb-3">
                        <b-row>
                           <b-col cols="12">
                              ...
                              <span v-if="chave.startsWith('t')" class="float-right">{{ parametros?.[`ta${chave.substr(1, 4)}`] }} °C</span>
                           ...
                           <b-col cols="12">
                              ...
                              <span v-if="chave.startsWith('t')" class="float-right">{{ parametros?.[`tb${chave.substr(1, 4)}`] }} °C</span>
            ...
            <b-row v-show="filtroGrafico == 'acionamento'">
               <b-col>
                  <b-table-simple hover small class="mt-2">
                     ...
                     <b-tbody>
                        ...
                        <b-tr>
                           <b-td>Carga</b-td>
                           <b-td>{{ this.acionamentos?.cargaTempo || 0 }} h</b-td>
                        </b-tr>
                        ...
                        <b-tr v-if="this.acionamentos?.minimo3Tempo != null">
                           <b-td>Nível mínimo 3</b-td>
                           <b-td>{{ this.acionamentos?.minimo3Tempo || 0 }} h</b-td>
                        </b-tr>
                        <b-tr v-if="this.acionamentos?.maximo3Tempo != null">
                           <b-td>Nível máximo 3</b-td>
                           <b-td>{{ this.acionamentos?.maximo3Tempo || 0 }} h</b-td>
                        </b-tr>
                        <b-tr v-if="this.acionamentos?.minimo4Tempo != null">
                           <b-td>Nível mínimo 4</b-td>
                           <b-td>{{ this.acionamentos?.minimo4Tempo || 0 }} h</b-td>
                        </b-tr>
                        <b-tr v-if="this.acionamentos?.maximo4Tempo != null">
                           <b-td>Nível máximo 4</b-td>
                           <b-td>{{ this.acionamentos?.maximo4Tempo || 0 }} h</b-td>
                        </b-tr>
                     </b-tbody>
                  </b-table-simple>
               </b-col>
            ...
            <b-row>
               <b-col cols="12">
                  <b-table class="mt-2" thead-class="text-center" sort-by="data" small striped bordered responsive :items="umidades.dadosTabela" :fields="fields" v-if="filtroGrafico == 'umidade' && umidades.dadosTabela?.length" style="max-height: 300px">
...
export default {
   ...
   data() {
      return {
         ...
         chavesAcionamentos: [],
         verificacaoConfig: [
            { objeto: 'temperaturas', props: ['t1', 't2', 't3'] },
            { objeto: 'pressoes', props: ['p1', 'p2', 'p3', 'p4', 'p5'] },
            { objeto: 'umidades', props: ['ue', 'us', 'di'] },
            { objeto: 'acionamentos', props: ['minimo1', 'maximo1', 'descarga', 'exaustor', 'elevador', 'transportador'] }
         ],
         fields: [
   ...
   computed: {
      ...
      chavesParametros() {
         return Object.keys(this.parametros)
            .filter((chave) => (chave.startsWith('t') || chave.startsWith('p')) && !chave.startsWith('to') && !chave.startsWith('ta') && !chave.startsWith('tb') && !chave.startsWith('pa') && !chave.startsWith('pb'))
            .sort((a, b) => {
   ...
   methods: {
      ...
      acionaBusca(filtro) {
         this.variavel = null
         if (filtro == 'temperatura' || filtro == 'todos') {
            this.buscarTemperaturas()
         } else if (filtro == 'pressao') {
            this.buscarPressoes()
         } else if (filtro == 'umidade') {
            this.buscarUmidades()
         } else if (filtro == 'parametro') {
            this.buscarParametros()
         } else if (filtro == 'acionamento') {
      ...
      async buscarTemperaturas() {
         ...
         const dados = await client.get(`/secador/buscartemperatura/${this.idSecador}?inicio=${this.dataInicio}&fim=${this.dataFim}`).catch((err) => {
         ...
         if (dados && dados.data) {
            ...
            for (let key in converteTemperatura) {
               if (Object.prototype.hasOwnProperty.call(this.temperaturas, key)) {
                  this.temperaturas[converteTemperatura[key]] = this.temperaturas[key]
                  delete this.temperaturas[key]
               }
            }
            this.pressoes = {}
            this.umidades = {}
      ...
      async buscarPressoes() {
         ...
         const dados = await client.get(`/secador/buscarpressao/${this.idSecador}?inicio=${this.dataInicio}&fim=${this.dataFim}`).catch((err) => {
      ...
      async buscarUmidades() {
         ...
         const dados = await client.get(`/secador/buscarumidade/${this.idSecador}?inicio=${this.dataInicio}&fim=${this.dataFim}`).catch((err) => {
      ...
      async buscarParametros() {
         ...
         const dados = await client.get(`/secador/buscarparametro/${this.idSecador}`).catch((err) => {
         ...
         if (dados && dados.data) {
            ...
            for (let key in converteTemperatura) {
               if (Object.prototype.hasOwnProperty.call(this.parametros, key)) {
                  this.parametros[converteTemperatura[key]] = this.parametros[key]
      ...
      async buscarAcionamentos() {
         ...
         const dados = await client.get(`/secador/buscaracionamento/${this.idSecador}?inicio=${this.dataInicio}&fim=${this.dataFim}`).catch((err) => {
      ...
      construirGrafico(dados) {
         ...
         if (this.filtroGrafico == 'temperatura' || this.filtroGrafico == 'todos') {
            // legendaGrafico = ['S1', 'S2', 'S3']
            ...
            seriesGrafico = this.chavesTemperatura.map((chave) => ({
               ...
               type: 'line',
               showSymbol: false,
               data: dados[chave.replace(/s/g, 't')],
               tooltip: { valueFormatter: (value) => value + ' °C' }
            }))
            // seriesGrafico = [
            //    { name: 'S1', type: 'line', symbol: 'none', symbolSize: 8, data: dados?.t1 }
            // ]
         } else if (this.filtroGrafico == 'pressao') {
         ...
         } else if (this.filtroGrafico == 'umidade') {
            legendaGrafico = ['U1', 'U2']
            tituloGrafico = 'Umidades e capacidades'
            unidadeGrafico = 'U(%)'
            unidadeVariavel = '%'
            seriesGrafico = [
               { name: 'U1', type: 'line', showSymbol: false, data: dados?.ue, tooltip: { valueFormatter: (value) => value + ' %' } },
               { name: 'U2', type: 'line', showSymbol: false, data: dados?.us, tooltip: { valueFormatter: (value) => value + ' %' } }
            ]
         } else if (this.filtroGrafico == 'acionamento') {
            ...
            legendaGrafico = ['Exaustor', 'Transportador', 'Descarga', 'Elevador']
            unidadeGrafico = ''
            unidadeVariavel = ''
            seriesGrafico = [
               { name: 'Transportador', type: 'line', showSymbol: false, lineStyle: { width: 20, color: '#72BFE5' }, itemStyle: { color: '#72BFE5' }, data: this.acionamentos.transportador },
               { name: 'Descarga', type: 'line', showSymbol: false, lineStyle: { width: 20, color: '#3CA6DA' }, itemStyle: { color: '#3CA6DA' }, data: this.acionamentos.descarga },
               { name: 'Elevador', type: 'line', showSymbol: false, lineStyle: { width: 20, color: '#1E6FBA' }, itemStyle: { color: '#1E6FBA' }, data: this.acionamentos.elevador },
               { name: 'Exaustor', type: 'line', showSymbol: false, lineStyle: { width: 20, color: '#18559a' }, itemStyle: { color: '#18559a' }, data: this.acionamentos.exaustor },
         ...
         const formatTooltip = (params) => {
            ...
            if (this.filtroGrafico == 'acionamento') {
               const grouped = {}
         ...
         this.opcoesDoGrafico = {
            ...
            yAxis: { type: 'value', name: unidadeGrafico, splitNumber: 10, axisLabel: { show: this.filtroGrafico == 'acionamento' ? false : true } },
      ...
      graficoVariavel(variavel) {
         ...
         if (variavel.startsWith('s')) {
            if (this.modo === 'fluxo') {
               ...
               adicionarMarkArea(this.temperaturas.operacoes?.carga, 'rgba(254, 254, 49, 0.3)')
               adicionarMarkArea(this.temperaturas.operacoes?.carregado, 'rgba(247, 96, 28, 0.3)')
               adicionarMarkArea(this.temperaturas.operacoes?.secagem, 'rgba(77, 205, 60, 0.3)')
               adicionarMarkArea(this.temperaturas.operacoes?.descarga, 'rgba(64, 206, 253, 0.3)')
            }
            tituloGrafico = 'Temperaturas'
            unidadeGrafico = 'T(°C)'
            unidadeVariavel = '°C'
            ...
            if (this.modo === 'fluxo') {
               ...
               adicionarMarkArea(this.pressoes.operacoes?.carga, 'rgba(254, 254, 49, 0.3)')
               adicionarMarkArea(this.pressoes.operacoes?.carregado, 'rgba(247, 96, 28, 0.3)')
               adicionarMarkArea(this.pressoes.operacoes?.secagem, 'rgba(77, 205, 60, 0.3)')
               adicionarMarkArea(this.pressoes.operacoes?.descarga, 'rgba(64, 206, 253, 0.3)')
            }
            tituloGrafico = 'Pressões'
            unidadeGrafico = 'P(mmCa)'
            unidadeVariavel = 'mmCa'
            ...
            if (this.modo === 'fluxo') {
               ...
               adicionarMarkArea(this.umidades.operacoes?.desligado, 'transparent')
               adicionarMarkArea(this.umidades.operacoes?.carga, 'transparent')
               adicionarMarkArea(this.umidades.operacoes?.carregado, 'transparent')
               adicionarMarkArea(this.umidades.operacoes?.secagem, 'rgba(77, 205, 60, 0.3)')
               adicionarMarkArea(this.umidades.operacoes?.descarga, 'rgba(64, 206, 253, 0.3)')
            }
            tituloGrafico = 'Umidades e capacidades'
            unidadeGrafico = 'U(%)'
            unidadeVariavel = '%'
            legendaGrafico = ['U1', 'U2', 'Objetivo']
            seriesGrafico = [
               { name: 'U1', type: 'line', showSymbol: false, data: this.umidades.ue, markArea: { data: markAreasGrafico }, tooltip: { valueFormatter: (value) => value + ' %' } },
               { name: 'U2', type: 'line', showSymbol: false, data: this.umidades.us, tooltip: { valueFormatter: (value) => value + ' %' } },
            ...
            tituloGrafico = 'Umidades e capacidades'
            unidadeGrafico = 'ton/h'
            unidadeVariavel = 'ton/h'
            legendaGrafico = ['Produção']
            seriesGrafico = [{ name: 'Produção', type: 'line', showSymbol: false, data: this.umidades.di, markArea: { data: markAreasGrafico }, tooltip: { valueFormatter: (value) => value + ' ton/h' } }]
            media = this.umidades.di
         ...
         this.opcoesDoGrafico = {
            ...
            legend: { textStyle: { color: '#6c727a' }, top: 25, data: legendaGrafico },
            grid: { left: '3%', right: '4%', bottom: '10%', containLabel: true },
            xAxis: {
               type: 'time',
               boundaryGap: false,
      ...
      enviarVariavel(variavel, modo) {
         this.$emit('variavelSelecionada', [variavel, modo])
      },
      formatarData(value) {
         if (!value) return ' - '
         return this.$moment(value, 'YYYY/MM/DD HH:mm:ss').format('DD/MM/YYYY HH:mm:ss')
      },
      decimalParaBinario(decimal, bits) {
         if (!decimal) {
            decimal = 0
         }
         let binario = decimal.toString(2)
         binario = '0'.repeat(Math.max(bits - binario.length, 0)) + binario
         return binario
      },
