<template>
   <div class="midde_cont">
      <div class="col-md-12">
         <div class="full">
            ...
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-transacao" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-transacao" title="Excluir Transação?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_transacao }}) {{ itemSelecionado.nm_transacao }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_transacao" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarTransacao" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovaTransacao.nm_transacao" :state="validate('nm_transacao')"></b-form-input>
               <b-form-invalid-feedback id="nm_transacao">{{ erros['nm_transacao'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
            <b-form-group label="Código">
               <b-form-input v-model="objNovaTransacao.cd_transacao" :state="validate('cd_transacao')"></b-form-input>
               <b-form-invalid-feedback id="cd_transacao">{{ erros['cd_transacao'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_transacao', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'nm_transacao', label: 'Descrição' },
            { key: 'cd_transacao', label: 'Código' }
         ]
      }
   },
   ...
   computed: {
      ...
      itemsFiltrados() {
         ...
         return this.items.filter((i) => {
            ...
            keys.splice(keys.indexOf('id_transacao'), 1)
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovaTransacao = {}
         this.tituloModal = 'Cadastrar Transação'
         this.$bvModal.show('cadastro')
      },
      async salvarTransacao(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await transacaoService.salvarTransacao(this.objNovaTransacao)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovaTransacao = { ...this.itemSelecionado }
         this.erros = []
         this.tituloModal = 'Editar Transação'
         this.$bvModal.show('cadastro')
      },
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione uma transação na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const result = await client.delete(`/transacao/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Transação',
   ...
   watch: {
      'objNovaTransacao.cd_transacao'(newValue) {
         if (newValue) {
            this.objNovaTransacao.cd_transacao = newValue.replace(/\s/g, '').toUpperCase()
         }
      }
   },
