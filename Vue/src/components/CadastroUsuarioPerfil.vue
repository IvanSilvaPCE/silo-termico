<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Usuários perfis cadastrados</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="nm_usuario" hover selectable>
                  ...
                  <template #cell(usuarios_perfis)="row">
                     <b-button :disabled="!row.item.usuarios_perfis.length" variant="outline-primary" size="sm" title="Ver Perfis" @click="row.toggleDetails()"
                        ><b-icon-eye-fill class="mr-2"></b-icon-eye-fill><span>{{ row.item.usuarios_perfis.length }}</span></b-button
                     >
                  </template>
                  <template #row-details="row">
                     <b-card>
                        <b-row>
                           <b-col md="12">
                              <b-table :fields="fieldsDetalhe" :items="row.item.usuarios_perfis"></b-table>
                           </b-col>
                        </b-row>
                     </b-card>
                  </template>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarUsuarioPerfil" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Usuário">
               <b-overlay :show="overlayUsuarios" rounded="sm">
                  <v-select v-model="objNovoUsuarioPerfil.id_usuario" :options="optionsUsuarios" :state="validate('id_usuario')" :reduce="(usuario) => usuario.value" :class="{ 'border-danger': validate('id_usuario') == false }">
            ...
            <b-form-group label="Perfil">
               <b-overlay :show="overlayPerfis" rounded="sm">
                  <v-select multiple v-model="objNovoUsuarioPerfil.id_perfil" :options="optionsPerfis" :state="validate('id_perfil')" :reduce="(perfil) => perfil.value" :class="{ 'border-danger': validate('id_perfil') == false }">
                     <template #no-options="">Nenhuma correspondência</template>
                  </v-select>
...
import { usuarioPerfilService } from '../services'

export default {
   name: 'CadastroUsuarioPerfil',
   data() {
      return {
         ...
         fields: [
            ...
            { key: 'usuarios_perfis', label: 'Perfis' }
         ],
         fieldsDetalhe: [
            { key: 'nm_perfil', label: 'Perfil' },
            { key: 'cd_perfil', label: 'Cód. Perfil' }
         ]
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoUsuarioPerfil = {}
         this.tituloModal = 'Cadastrar Usuário Perfil'
         this.$bvModal.show('cadastro')
      },
      async salvarUsuarioPerfil(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await usuarioPerfilService.salvarUsuarioPerfil(this.objNovoUsuarioPerfil)
         switch (status) {
            case 200:
      ...
      editar() {
         const id_perfil = this.itemSelecionado.usuarios_perfis.map((perfis) => perfis.id_perfil)
         this.objNovoUsuarioPerfil = { ...this.itemSelecionado, id_perfil }
         this.erros = []
   ...
   mounted() {
      this.buscar(), this.buscarPerfis(), this.buscarUsuarios(), (this.totalRows = this.items.length)
   }
}
</script>
