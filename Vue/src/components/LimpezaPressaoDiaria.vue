<template>
   <b-overlay :show="buscandoPressao" rounded="sm" class="overlay">
      <b-row>
         <b-col sm="12" md="6">
            <b-form-datepicker v-model="data" @input="buscarPressao()" :date-info-fn="marcaDatasOperacao" :date-format-options="{ year: 'numeric', month: '2-digit', day: '2-digit' }" label-help="Use as setas para navegar pelas datas" label-no-date-selected="Nenhuma data selecionada"></b-form-datepicker>
         </b-col>
      </b-row>
         ...
         <b-col cols="12">
            <b-list-group horizontal class="mt-2">
               <b-list-group-item class="d-flex justify-content-between align-items-center" v-b-tooltip.hover title="Pressão média">
                  ...
                  <small class="valor">{{ this.pressao?.pressao && this.pressao.pressao.length ? (this.pressao.pressao.reduce((acc, val) => acc + (isNaN(parseFloat(val[1])) ? 0 : parseFloat(val[1])), 0) / this.pressao.pressao.length).toFixed(1) : '-' }}</small>
               ...
               <b-list-group-item class="d-flex justify-content-between align-items-center" v-b-tooltip.hover title="Pressão mínima">
                  ...
                  <small class="valor">{{ this.pressao?.pressao && this.pressao.pressao.length ? Math.min(...this.pressao.pressao.map((item) => parseFloat(item[1]))) : '-' }}</small>
                  <small class="ml-1">mmCa</small>
               </b-list-group-item>
               <b-list-group-item class="d-flex justify-content-between align-items-center" v-b-tooltip.hover title="Pressão máxima">
                  ...
                  <small class="valor">{{ this.pressao?.pressao && this.pressao.pressao.length ? Math.max(...this.pressao.pressao.map((item) => parseFloat(item[1]))) : '-' }}</small>
                  <small class="ml-1">mmCa</small>
               </b-list-group-item>
            </b-list-group>
         </b-col>
...
export default {
   ...
   methods: {
      ...
      async buscarPressao() {
         ...
         const dados = await client.get(`/limpeza/buscarpressaodiaria/${this.idLimpeza}?data=${this.data}`).catch((err) => {
            this.$bvToast.toast(`Erro ao buscar pressão diária. ${err}`, {
               title: 'Erro',
               variant: 'warning',
               autoHideDelay: 5000,
               solid: true,
