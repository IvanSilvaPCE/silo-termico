<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Marcas de componentes cadastradas</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-marca" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-marca" title="Excluir Marca?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_marca }}) {{ itemSelecionado.ds_marca }}
                              <hr />
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_marca" hover selectable>
                  <template #table-busy>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarMarca" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Descrição">
               <b-form-input v-model="objNovaMarca.ds_marca" :state="validate('ds_marca')"></b-form-input>
               <b-form-invalid-feedback id="ds_marca">{{ erros['ds_marca'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_marca', label: 'ID', tdClass: 'text-center', class: 'text-center' },
   ...
   computed: {
      ...
      itemsFiltrados() {
         ...
         return this.items.filter((i) => {
            ...
            keys.splice(keys.indexOf('id_marca'), 1)
   ...
   methods: {
      ...
      async salvarMarca(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await marcaService.salvarMarca(this.objNovaMarca)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovaMarca = { ...this.itemSelecionado }
         this.erros = []
      ...
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione uma marca na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_marca
         const result = await client.delete(`/marca/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Marca',
