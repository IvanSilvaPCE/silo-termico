...
export default {
  ...
  data() {
    return {
      ...
      // 🎯 Estados para drag and drop (baseado em ModeladorSVG.vue)
      isDragging: false,
      dragElement: null,
      dragType: null, // 'pendulo' ou 'sensor'
      dragOffset: { x: 0, y: 0 },
      saveTimeout: null, // Para debounce do salvamento
      _globalsAdded: false // Flag para controlar listeners globais
    }
  },
  computed: {
    dimensoesCalculadas() {
  ...
  mounted() {
    ...
    // 🎯 Adicionar event listeners após SVG ser renderizado
    this.$nextTick(() => {
      setTimeout(() => {
  ...
  beforeDestroy() {
    // 🎯 Limpar event listeners e timers ao destruir componente
  ...
  methods: {
    updateSVG() {
      ...
      // 🎯 CRITICAL: Re-bind event listeners after SVG re-render
      this.$nextTick(() => {
        // Allow DOM to update
        setTimeout(() => {
          this.adicionarEventListeners()
    ...
    // 🎯 MISSING METHODS: Add required methods
    recalcularDimensoes() {
      return this.calcularDimensoesBaseadoNoFundo()
    },

    salvarDimensoesNoModelo(novasDimensoes) {
      console.log('📐 [ArmazemSvg] Salvando dimensões:', novasDimensoes)
      // Emit to parent to update model
      this.$emit('dimensoes-atualizadas', novasDimensoes)
    },
    ...
    // =====================================
    // 🎯 MÉTODOS DE DRAG AND DROP (baseados em ModeladorSVG.vue)
    // =====================================

    // 🎯 Adicionar event listeners de forma idempotente  
    adicionarEventListeners() {
      ...
      this.$nextTick(() => {
        ...
        setTimeout(() => {
          ...
          // Gerenciar listeners globais de forma idempotente
          this.garantirListenersGlobais()
        }, 50)
      })
    },

    // 🔧 Método para garantir listeners globais sem duplicação
    ...
    iniciarDragPendulo(event, numeroPendulo) {
      ...
      console.log(`🎯 [ArmazemSvg] Iniciando drag pêndulo ${numeroPendulo}`)

      this.isDragging = true
      this.dragType = 'pendulo'
      this.dragElement = numeroPendulo

      // 🔧 IMPROVED: Use getScreenCTM().inverse() for accurate SVG coordinates
      const svg = event.target.closest('svg')
      const svgPoint = this.getAccurateSVGPoint(svg, event.clientX, event.clientY)
      ...
      if (elementoPrincipal) {
        ...
        const largura = parseFloat(elementoPrincipal.getAttribute('width')) || 16
        const altura = parseFloat(elementoPrincipal.getAttribute('height')) || 16

        this.dragOffset = {
          x: svgPoint.x - (penduloX + largura / 2),
          y: svgPoint.y - (penduloY + altura / 2)
        }
      } else {
    ...
    iniciarDragSensor(event, numeroPendulo, numeroSensor) {
      ...
      console.log(`🎯 [ArmazemSvg] Iniciando drag sensor ${numeroPendulo}-${numeroSensor}`)
      ...
      if (elementoPrincipal) {
        ...
        this.dragOffset = {
          x: svgPoint.x - (sensorX + largura / 2),
          y: svgPoint.y - (sensorY + altura / 2)
        }
      } else {
        this.dragOffset = { x: 0, y: 0 }
      }
    ...
    continuarDrag(event) {
      ...
      // 🔧 IMPROVED: Use accurate SVG coordinate system
      const svgPoint = this.getAccurateSVGPoint(svg, event.clientX, event.clientY)

      // 🎯 VALIDATE: Check bounds relative to warehouse base (gray area)
      const boundingInfo = this.getWarehouseBounds()
      const novaX = Math.max(boundingInfo.xMin, Math.min(boundingInfo.xMax, svgPoint.x - this.dragOffset.x))
      const novaY = Math.max(boundingInfo.yMin, Math.min(boundingInfo.yMax, svgPoint.y - this.dragOffset.y))
    ...
    finalizarDrag(event) {
      ...
      console.log(`🎯 [ArmazemSvg] Finalizando drag ${this.dragType}`)
      ...
      // Readicionar event listeners após finalizar drag
      this.$nextTick(() => {
        setTimeout(() => {
          this.adicionarEventListeners()
        }, 100)
    ...
    // 🎯 ENHANCED: Debounced save with complete position data
    debouncedSalvarPosicoes(elementoMovido, tipoMovido) {
      ...
      this.saveTimeout = setTimeout(() => {
        console.log(`💾 [ArmazemSvg] Salvando posições - tipo: ${tipoMovido}`)

        // 🎯 COLLECT: Get all current positions for full state
        const posicoesPendulos = this.coletarPosicoesPendulos()
        const posicoesSensores = this.coletarPosicoesSensores()

        // Emitir evento completo para o componente pai
    ...
    moverPenduloCompletoSemUpdate(numeroPendulo, novaX, novaY) {
      ...
      const posicaoOriginal = this.calcularPosicaoOriginalPendulo(numeroPendulo, 
        this.config.quantidadePendulos || 3, 
        this.dimensoesCalculadas.largura)
      
      const offsetX = novaX - posicaoOriginal.x
      const offsetY = novaY - posicaoOriginal.y

      // Atualizar visualmente o pêndulo principal
      const elementoPendulo = document.getElementById(`C${numeroPendulo}`)
      const textoPendulo = document.getElementById(`TC${numeroPendulo}`)
      
      if (elementoPendulo) {
        const largura = parseFloat(elementoPendulo.getAttribute('width')) || 16
        elementoPendulo.setAttribute('x', novaX - largura / 2)
        elementoPendulo.setAttribute('y', novaY)
      }
      
      if (textoPendulo) {
        textoPendulo.setAttribute('x', novaX)
        textoPendulo.setAttribute('y', novaY + 8)
      }

      // Atualizar visualmente todos os sensores deste pêndulo
      const config = this.config
      const sensoresPorPendulo = config.sensoresPorPendulo || {}
      const numSensores = sensoresPorPendulo[numeroPendulo] || 3
      const dist_y_sensores = config.dist_y_sensores || 12
      const afastamento_vertical_pendulo = config.afastamento_vertical_pendulo || 0

      for (let s = 1; s <= numSensores; s++) {
        const ySensorBase = novaY - dist_y_sensores * s - 25 - afastamento_vertical_pendulo
        
        const elementoSensor = document.getElementById(`C${numeroPendulo}S${s}`)
        const textoSensor = document.getElementById(`TC${numeroPendulo}S${s}`)
        const nomeSetor = document.getElementById(`TIND${numeroPendulo}S${s}`)
        
        if (elementoSensor) {
          const largura = parseFloat(elementoSensor.getAttribute('width')) || 16
          elementoSensor.setAttribute('x', novaX - largura / 2)
          elementoSensor.setAttribute('y', ySensorBase)
        }
        
        if (textoSensor) {
          textoSensor.setAttribute('x', novaX)
          textoSensor.setAttribute('y', ySensorBase + 8)
        }
        
        if (nomeSetor) {
          nomeSetor.setAttribute('x', novaX - 10)
          nomeSetor.setAttribute('y', ySensorBase + 8)
        }
      }

      console.log(`📍 [ArmazemSvg] Pêndulo ${numeroPendulo} movido - offset: (${offsetX.toFixed(1)}, ${offsetY.toFixed(1)})`)
    },

    moverSensorIndividualSemUpdate(numeroPendulo, numeroSensor, novaX, novaY) {
      // Calcular diferença da posição original do sensor
      ...
      // Atualizar visualmente apenas este sensor
      const elementoSensor = document.getElementById(`C${numeroPendulo}S${numeroSensor}`)
      const textoSensor = document.getElementById(`TC${numeroPendulo}S${numeroSensor}`)
      const nomeSetor = document.getElementById(`TIND${numeroPendulo}S${numeroSensor}`)
      ...
      if (textoSensor) {
        textoSensor.setAttribute('x', novaX)
        textoSensor.setAttribute('y', novaY + 8)
      }
      
      if (nomeSetor) {
        nomeSetor.setAttribute('x', novaX - 10)
        nomeSetor.setAttribute('y', novaY + 8)
      }

      console.log(`📍 [ArmazemSvg] Sensor ${numeroPendulo}-${numeroSensor} movido - offset: (${offsetX.toFixed(1)}, ${offsetY.toFixed(1)})`)
    },

    // 🎯 NEW: Accurate SVG coordinate transformation using getScreenCTM()
    getAccurateSVGPoint(svg, clientX, clientY) {
      ...
      try {
        ...
        console.warn('🔧 [ArmazemSvg] getScreenCTM() fallback to manual conversion')
      }
      
      // Fallback to manual conversion if getScreenCTM() fails
      return this.converterParaCoordenadaSVG(svg, clientX, clientY)
    },

    // 🎯 NEW: Get warehouse base bounds for drag validation
    getWarehouseBounds() {
      const config = this.config
      const lb = config.lb || 350
      const pb = config.pb || 185
      const le = config.le || 15
      const lf = config.lf || 250
      const hb = config.hb || 30
      const ht = config.ht || 50
      
      // Calculate warehouse base area (gray zone)
      return {
        xMin: le + 10, // Left edge + margin
        xMax: lb - le - 10, // Right edge - margin  
        yMin: pb - ht - 20, // Top area - margin
        yMax: pb - hb + 10 // Bottom area + margin
      }
    },

    // 🎯 NEW: Collect all current pendulum positions
    coletarPosicoesPendulos() {
      const posicoes = {}
      const quantidadePendulos = this.config.quantidadePendulos || 3
      
      for (let i = 1; i <= quantidadePendulos; i++) {
        const elemento = document.getElementById(`C${i}`)
        if (elemento) {
          const x = parseFloat(elemento.getAttribute('x')) || 0
          const y = parseFloat(elemento.getAttribute('y')) || 0
          const largura = parseFloat(elemento.getAttribute('width')) || 16
          const altura = parseFloat(elemento.getAttribute('height')) || 16
          
          // Calculate offset from original position
          const posicaoOriginal = this.calcularPosicaoOriginalPendulo(i, quantidadePendulos, this.dimensoesCalculadas.largura)
          
          posicoes[i] = {
            x: (x + largura / 2) - posicaoOriginal.x,
            y: (y + altura / 2) - posicaoOriginal.y
          }
        }
      }
    ...
    // 🎯 NEW: Collect all current sensor positions
    coletarPosicoesSensores() {
      const posicoes = {}
      const quantidadePendulos = this.config.quantidadePendulos || 3
      const sensoresPorPendulo = this.config.sensoresPorPendulo || {}
      
      for (let p = 1; p <= quantidadePendulos; p++) {
        const numSensores = sensoresPorPendulo[p] || 3
        
        for (let s = 1; s <= numSensores; s++) {
          const elemento = document.getElementById(`C${p}S${s}`)
          if (elemento) {
            const x = parseFloat(elemento.getAttribute('x')) || 0
            const y = parseFloat(elemento.getAttribute('y')) || 0
            const largura = parseFloat(elemento.getAttribute('width')) || 16
            ...
            // Calculate offset from original position
            const posicaoOriginal = this.calcularPosicaoOriginalSensor(p, s, quantidadePendulos, this.dimensoesCalculadas.largura)
