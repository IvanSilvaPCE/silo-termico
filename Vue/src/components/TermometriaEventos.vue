         ...
         <div class="heading1 margin_0">
            <b-row>
               ...
               <b-col cols="2" class="text-right">
                  ...
                  <b-button variant="outline-primary" size="sm" v-b-tooltip.hover title="Ver log de eventos" @click="verLog(idTermometria)"><b-icon-clock></b-icon-clock></b-button>
               </b-col>
            </b-row>
         </div>
      </div>
      <div class="map_section padding_infor_info">
         <b-table :items="items" :fields="fields" :busy="isBusy" select-mode="single" responsive="sm" sort-by="tp_criticidade_alarme" sort-desc hover selectable striped>
            <template #table-busy>
...
export default {
   ...
   methods: {
      async buscar() {
         ...
         const dados = await client.get(`/termometria/buscaralarme/${this.idTermometria}?dia=${dia}`).catch((err) => {
   ...
   mounted() {
      const idTermometriaLocalStorage = localStorage.getItem('ter')
