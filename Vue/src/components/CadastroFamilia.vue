<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Famílias de produtos cadastradas</h2>
               </div>
            </div>
               ...
               <div class="row">
                  <b-col class="my-1" md="6">
                     <b-button-group class="mb-4">
                        ...
                        <b-button id="popover-excluir-familia" variant="outline-danger" @click="confirmarExclusao" :disabled="!temAlgumSelecionado"> <b-icon-trash></b-icon-trash>Excluir </b-button>
                        <b-popover variant="" triggers="manual" :show.sync="popovers.excluir" target="popover-excluir-familia" title="Excluir Família?">
                           <div class="mt-2" v-if="selecionado.length > 0">
                              ({{ itemSelecionado.id_familia }}) {{ itemSelecionado.nm_familia }}
                              <hr />
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarFamilia" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Nome da Familia">
               <b-form-input v-model="objNovaFamilia.nm_familia" :state="validate('nm_familia')"></b-form-input>
               <b-form-invalid-feedback id="nm_familia">{{ erros['nm_familia'] | parseErros }}</b-form-invalid-feedback>
            </b-form-group>
         </form>
...
export default {
   ...
   data() {
      return {
         ...
         fields: [
            { key: 'id_familia', label: 'ID', tdClass: 'text-center', class: 'text-center' },
            { key: 'nm_familia', label: 'Nome Família' }
         ]
      }
   },
   ...
   computed: {
      ...
      itemsFiltrados() {
         ...
         return this.items.filter((i) => {
            ...
            keys.splice(keys.indexOf('id_familia'), 1)
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovaFamilia = {}
         this.tituloModal = 'Cadastrar Família'
         this.$bvModal.show('cadastro')
      },
      async salvarFamilia(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await familiaService.salvarFamilia(this.objNovaFamilia)
         switch (status) {
            case 200:
      ...
      editar() {
         this.objNovaFamilia = { ...this.itemSelecionado }
         this.erros = []
      ...
      confirmarExclusao() {
         if (this.temAlgumSelecionado) {
            ...
            this.showMessage({
               title: 'Erro!',
               msg: 'Primeiro selecione uma família na lista',
               variant: 'warning'
            })
         }
      },
      async excluir() {
         ...
         const id = this.itemSelecionado.id_familia
         const result = await client.delete(`/familia/${id}`)
         if (result.data == true) {
            this.showMessage({
               title: 'Família',
