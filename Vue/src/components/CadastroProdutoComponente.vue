<template>
   <div class="midde_cont">
         ...
         <div class="full">
            <div class="full graph_head">
               <div class="heading1 margin_0">
                  <h2>Produtos componentes cadastrados</h2>
               </div>
            </div>
            <div class="table_section padding_infor_info">
               ...
               <b-table :items="itemsFiltrados" :fields="fields" :busy="isBusy" :current-page="currentPage" :per-page="perPage" @row-selected="selecionado = $event" select-mode="single" responsive="sm" sort-by="id_produto" hover selectable>
                  ...
                  <template #cell(componentes)="row">
                     <b-button :disabled="!row.item.produtos_componentes.length" variant="outline-primary" size="sm" title="Ver Componentes" @click="row.toggleDetails()">
                        <b-icon-eye-fill class="mr-2"></b-icon-eye-fill><span>{{ row.item.produtos_componentes.length }}</span>
                     </b-button>
                  </template>
                  <template #row-details="row">
                     <b-card>
                        <b-row>
                           <b-col md="12">
                              <b-table :fields="fieldsDetalhe" :items="row.item.produtos_componentes"></b-table>
                           </b-col>
                        </b-row>
                     </b-card>
                  </template>
      ...
      <b-modal id="cadastro" ref="modal" :title="tituloModal" @ok="salvarProdutoComponente" @cancel="resetModal" ok-title="Salvar" cancel-title="Cancelar">
         <form>
            <b-form-group label="Produto">
               <b-overlay :show="isBusy" rounded="sm">
                  <v-select v-model="objNovoProdutoComponente.id_produto" :options="optionsProdutos" :state="validate('id_produto')" :reduce="(produto) => produto.value" :class="{ 'border-danger': validate('id_produto') == false }">
            ...
            <b-form-group label="Componentes">
               <b-overlay :show="overlayComponentes" rounded="sm">
                  <v-select multiple v-model="objNovoProdutoComponente.id_componente" :options="optionsComponentes" :state="validate('id_componente')" :reduce="(componente) => componente.value" :class="{ 'border-danger': validate('id_componente') == false }">
                     <template #no-options="">Nenhuma correspondência</template>
                  </v-select>
                  <small class="text-danger">{{ erros['id_componente'] | parseErros }}</small>
               </b-overlay>
            </b-form-group>
         </form>
...
import { produtoComponenteService } from '../services'

export default {
   name: 'CadastroProdutoComponente',
   data() {
      return {
         overlayComponentes: false,
         componente: [],
         filter: null,
         tituloModal: '',
         selecionado: [],
   ...
   computed: {
      ...
      optionsComponentes() {
         if (!this.componente.length) {
            return
         }
         return this.componente.map((componente) => ({
            value: componente.id_componente,
            label: componente.ds_componente
         }))
      }
   },
   ...
   methods: {
      formNovo() {
         this.erros = []
         this.objNovoProdutoComponente = {}
         this.tituloModal = 'Cadastrar Produto Componente'
         this.$bvModal.show('cadastro')
      },
      async salvarProdutoComponente(bvModalEvt) {
         this.isSalvando = true
         bvModalEvt.preventDefault()
         let msg = ''
         const { status, data } = await produtoComponenteService.salvarProdutoComponente(this.objNovoProdutoComponente)
         switch (status) {
            case 200:
      ...
      async buscarComponentes() {
         this.overlayComponentes = true
         const dados = await client.get('/componente').catch((err) => {
            this.$bvToast.toast(`Erro ao buscar componentes. ${err}`, {
               title: 'Erro',
               variant: 'warning',
               autoHideDelay: 5000,
               solid: true,
         ...
         this.overlayComponentes = false
         this.componente = dados.data
      },
      editar() {
         const id_componente = this.itemSelecionado.produtos_componentes.map((componentes) => componentes.id_componente)
         this.objNovoProdutoComponente = { ...this.itemSelecionado, id_componente }
         this.erros = []
         this.tituloModal = 'Editar Produto Componente'
         this.$bvModal.show('cadastro')
      },
      resetModal() {
         this.objNovoProdutoComponente = {}
